...
#undef MSG_HEAD
#define MSG_HEAD ("UseComuServ_SetIO")

int main()
{
	unsigned char ucIOSta = IO_APPLEDPOW_ON;
	int iVal = 0;
	bool bSymb = true;
	while( !IOSet(IOS_APPLEDPOW, ucIOSta, NULL, 0) )
	{
		bSymb = !bSymb;
		if( bSymb ) ucIOSta = IO_APPLEDPOW_ON;
		else ucIOSta = IO_APPLEDPOW_OFF;

		PRTMSG( MSG_NOR, "IO Set: No.%d, Sta-%d\n", IOS_APPLEDPOW, ucIOSta );
