...
#define USE_PROTOCOL	2	// 0,千里眼通用版协议; 1,千里眼研三公开版协议; 2,千里眼研一公开版协议(即公司公开版); 30,两客通用版协议
...
#define USE_VIDEO_TCP	0		// 1, 视频指令使用独立的套接字，0，不使用独立的套接字
// } 公共开关

// QianExe使用
#define USE_LIAONING_SANQI	0	// 1, 使用辽宁移动三期协议，0, 不使用
#define USE_BLK			1		// 1,支持黑匣子; 0,不支持
#define USE_AUTORPT		1		// 1,支持主动上报(0154); 0,不支持
...
// 】若USE_DRVREC为1，则这几条不要启用
#define USE_OIL			0		// 1,油耗数据使用IO直接读取; 2,使用油耗检测盒,并使用研一算法; 3,使用150pd02-003油耗检测盒; 0,不支持
...
#define VIDEO_BLACK_TYPE	2	// 1, 视频黑匣子上传使用旧协议，2, 使用新协议
...
#if CHK_VER == 0
	#define SOFT_VER		"c212d-110v8w001-103"
#else
	...
	#define SOFT_VER		"c212d-110v8w001-bkp"
...
#define QTCPPORT	17777				// 千里眼TCP端口
#if USE_LIAONING_SANQI == 1										//
...
#define DUDPPORT	31234				// DVR视频的UDP端口
...
struct tag2DIpPortPar
{
	...
	void Init()
	{
		...
		strcpy(m_szPhoneMod, "SIM5218"); 	strcpy(m_szTelNum, "");
