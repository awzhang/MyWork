...
bool CMd5::PrintMD5(uchar md5Digest[16], char *md5)
{
	char chBuffer[256];
	char chEach[10];
	int nCount;
...
bool CMd5::MD5String(char* szString, BYTE nLen,char *md5)
{
//	int nLen = strlen(szString);

	Update((uchar*)szString, (unsigned int)nLen);
	Finalize();
...
bool CMd5::MD5File(char* szFilename, char *md5)
{
	FILE* file;
	int nLen;
	uchar chBuffer[1024];
...
bool CMd5::MD5File16(char* szFilename, uchar *md5)
{
	FILE* file;
	int nLen;
	uchar chBuffer[1024];
