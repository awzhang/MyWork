...
//#define VPP_DEBUG
/* max number of cover region in one VI channle */
#define MAX_COVER_NUM     4  
/* max number of overlay region in one VENC channle */
#define MAX_OVERLAY_NUM   4  

/* max number of soft overlay region in one VENC channle */
#define MAX_SOVERLAY_NUM   4 

#define MAX_MOSAIC_REGION_NUM 4
...
/*all soft overlay region number*/
#define VI_SOVERLAY_REGION (VIU_MAX_DEV_NUM * VIU_MAX_CHN_NUM_PER_DEV *MAX_SOVERLAY_NUM)

/*all mosaic region number*/
#define VI_MOSAIC_REGION (VIU_MAX_DEV_NUM * VIU_MAX_CHN_NUM_PER_DEV *MAX_MOSAIC_REGION_NUM)
...
/* scale filter (Notes:225M means 2.25M)*/
typedef enum hiVPP_SCALE_FILTER_E
{
    VPP_SCALE_FILTER_DEFAULT = 0,/* default value */
    VPP_SCALE_FILTER_2M,
    VPP_SCALE_FILTER_225M,
    VPP_SCALE_FILTER_25M,
    VPP_SCALE_FILTER_275M,
    VPP_SCALE_FILTER_3M,
    VPP_SCALE_FILTER_325M,
    VPP_SCALE_FILTER_35M,
    VPP_SCALE_FILTER_375M,
    VPP_SCALE_FILTER_4M,
    VPP_SCALE_FILTER_5M,
    VPP_SCALE_FILTER_6M,
...
typedef struct hiVIDEO_PREPROC_CONF_S
{
    ...
    */
	...
	VPP_SCALE_FILTER_E enFilter;    /*filter option*/
...
typedef enum hiREGION_TYPE_E
{
    ...
    SOFTOVERLAY_REGION,  /* video soft overlay region*/
    MOSAIC_REGION,       /* video mosaic region*/
    REGION_BUTT
} REGION_TYPE_E;
...
typedef struct hiSOFT_OVERLAY_ATTR_S
{
    ...
    ** RGB format should be used for this parament.now only support ARGB1555!
    */
	HI_U32 u32BgColor;
   ...
   /* background an foreground transparence when the bitmap pixel format is ARGB1555  
    * the bitmap pixel format is ARGB1555,when the alpha bit is 1 this alpha is value!
    */
    HI_U32 u32FgAlpha;

	/* background an foreground transparence when the bitmap pixel format is ARGB1555 or others
    * the bitmap pixel format is ARGB1555,when the alpha bit is 0 this alpha is value!
    */
    HI_U32 u32BgAlpha;
...
typedef struct hiMOSAIC_ATTR_S
{
	...
	/*bitmap pixel format,now only support semi_plan 420 or semi_plan422*/
	PIXEL_FORMAT_E enPixelFmt;
...
typedef union hiREGION_ATTR_U
{
	...
	SOFT_OVERLAY_ATTR_S stSOverlay; /* attribute of soft overlay region*/
...
typedef enum hiREGION_CRTL_CODE_E
{
    ...
    REGION_GET_ALL_SOFT_OVERLAY_ATTR,  /* get attribute of all soft overlay regions */
}REGION_CRTL_CODE_E;
...
{       
    HI_U32 u32SoftOverlayNum;/*soft overlay region number*/
    REGION_HANDLE aSoftOverlayHandles[VI_SOVERLAY_REGION];/*soft overlay region handle*/
    SOFT_OVERLAY_ATTR_S astAttr[VI_SOVERLAY_REGION];/*soft overlay region handle*/
}SOFT_OVERLAY_S;

typedef struct hiMOSAIC_S
{       
    HI_U32 u32MosaicNum;/*mosaic region number*/
    REGION_HANDLE aMosaicHandles[VI_MOSAIC_REGION];/*mosaic region handle*/
    MOSAIC_ATTR_S astAttr[VI_MOSAIC_REGION];/*mosaic region handle*/
}MOSAIC_S;
...
typedef union hiREGION_CTRL_PARAMETER_U
{
    ...
    /* for REGION_SET_ALPHA0 & REGION_SET_ALPHA1 */
    HI_U32 u32Alpha;
	...
	/* for REGION_GET_ALL_SOVERLAY_ATTR */
