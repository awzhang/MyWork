...
#define HI_ERR_AENC_NOT_SURPPORT      HI_DEF_ERR(HI_ID_AENC, EN_ERR_LEVEL_ERROR, EN_ERR_NOT_SURPPORT)
...
#define HI_TRACE_AENC(level, fmt...) do{ \
		HI_TRACE(level, HI_ID_AENC,"\n[Func]:%s [Line]:%d [Info]:", __FUNCTION__, __LINE__);\
        HI_TRACE(level, HI_ID_AENC, fmt); \
}while(0)
